...
class Datasets:
    def __init__(self, speed_flag, mode, train_rate):
        ...
        self.data_List = []
        self.toTensor = vision.py_transforms.ToTensor()
...
def create_Dataset(batch_size, shuffle=True, speed_flag=False, mode="train", train_rate=0.8):
    datasets = Datasets(speed_flag, mode, train_rate)
    datasets = ds.GeneratorDataset(datasets, ["images", "labels", "task_flags"], shuffle=shuffle, num_parallel_workers=config.num_parallel_workers)
    datasets = datasets.batch(batch_size=batch_size, drop_remainder=True, num_parallel_workers=config.num_parallel_workers)
    return datasets
