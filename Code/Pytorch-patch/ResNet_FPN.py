...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, scale=1, input_channel=6):
        self.inplanes = 64
        ...
        if self.input_channel == 3:
            ...
            self.conv0 = nn.Conv2d(input_channel, 64, kernel_size=7, stride=2, padding=3, bias=False)
...
def resnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pretrained_model = models.resnet101(weights=models.ResNet101_Weights.IMAGENET1K_V2).state_dict()
        state = model.state_dict()
        for key in state.keys():
            if key in pretrained_model.keys():
                state[key] = pretrained_model[key]
...
if __name__ == '__main__':
    model = resnet50(pretrained=True, num_classes=2, input_channel=6)
    print(model)
